...
export default function TeamsOverview(): React.ReactElement {
  return (
    <section className={styles.teamsOverviewSection}>
      <h2>Meet Our Teams</h2>
      <div className={styles.teamsGrid}>
        {teams.map((team) => (
          <div className={styles.teamCard} key={team.name}>
            <img src={team.logo} alt={team.name + " Logo"} />
            <h3>{team.name}</h3>
            <p>{team.description}</p>
            <div className={styles.founded}>Founded: {team.founded}</div>
            <a href={team.links} className={styles.teamLink}>Learn More</a>
          </div>
        ))}
      </div>
    </section>
